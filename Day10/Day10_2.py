...
shoelaceCoordinates.append((sx,sy))
new_matrix[sx,sy] = 'S'
...
shoelaceCoordinates.append((nx,ny))
totalSteps += 1
...
for i in range(len(shoelaceCoordinates)-1):
    x1, y1 = shoelaceCoordinates[i]
    x2, y2 = shoelaceCoordinates[i+1]
    area += ((x1 * y2) - (x2 * y1))

area = abs(area) / 2 - (len(shoelaceCoordinates)/2-1)
